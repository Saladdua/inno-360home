...
export default function LoginModal({ onClose }: LoginModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === "auth/invalid-credential") {
        ...
        setLocalError("Tài khoản đã bị vô hiệu hóa")
      } else if (error.code === "auth/user-not-found") {
        setLocalError("Không tìm thấy tài khoản với email này")
      } else if (error.code === "auth/wrong-password") {
        setLocalError("Mật khẩu không đúng")
      } else {
        setLocalError("Đăng nhập không thành công. Vui lòng thử lại sau.")
      }
    } finally {
      setLoading(false)
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setLocalError("Đăng nhập với Google không thành công. Vui lòng thử lại sau.")
    } finally {
      setLoading(false)
    }
  }
